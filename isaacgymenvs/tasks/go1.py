...
class Go1(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.base_height_target = self.cfg["env"]["learn"]["baseHeightTarget"]
        ...
        self.init_done = True
        self.actuator_type = self.cfg['env']['control']['actuator']
        if self.actuator_type == 'NN':
            actuator_path = "/home/qi/Documents/github/IsaacGymEnvs/assets/urdf/go1/unitree_go1.pt"
            ...
            self.joint_pos_err = torch.zeros((self.num_envs, 12), device=self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        rigid_shape_prop = self.gym.get_asset_rigid_shape_properties(go1_asset)
        ...
        for i in range(self.num_envs):
            ...
            self.gym.set_asset_rigid_shape_properties(go1_asset, rigid_shape_prop)
            go1_handle = self.gym.create_actor(env_handle, go1_asset, start_pose, "go1", i, 0, 0)
        ...
        for i in range(len(knee_names)):
            self.knee_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.go1_handles[0], knee_names[i])

        self.base_index = self.gym.find_actor_rigid_body_handle(self.envs[0], self.go1_handles[0], "base")
    ...
    def check_termination(self):
        ...
        self.reset_buf = torch.where((self.progress_buf >= self.max_episode_length - 1) & ((self.common_step_counter-1)%24==0), torch.ones_like(self.reset_buf), self.reset_buf)

    def compute_observations(self):
        self.measured_heights = self.get_heights()
        heights = torch.clip(self.root_states[:, 2].unsqueeze(1) - self.base_height_target - self.measured_heights, -1, 1.) * self.height_meas_scale
    ...
    def compute_reward(self):
        ...
        # base height penalty
        rew_base_height = torch.square(self.root_states[:, 2] - self.base_height_target) * self.rew_scales["base_height"] # TODO add target base height to cfg

        # torque penalty
        #rew_torque = torch.sum(torch.square(self.torques), dim=1) * self.rew_scales["torque"]
        rew_torque = torch.sum(torch.abs(self.torques), dim=1) * self.rew_scales["torque"]
    ...
    def compute_torque(self, tgt_pos):
        if self.actuator_type == "PD":
            torques = self.Kp*(tgt_pos - self.dof_pos) - self.Kd*self.dof_vel
        elif self.actuator_type == "NN":
            self.joint_pos_err = self.dof_pos - tgt_pos
            ...
            raise Exception("actuator type is not recognized: " + self.actuator_type)

        torques = torch.clip(torques, -35.0, 35.0) ##todo:bound with max for each joint
        return torques

    def pre_physics_step(self, actions):
        self.actions = actions.clone().to(self.device)

        tgt_pos = self.action_scale * self.actions + self.default_dof_pos
        for i in range(self.decimation):
            torques = self.compute_torque(tgt_pos)
    ...
    def curiculum(self, num_robots, num_terrains, num_levels):
        ...
        for j in range(num_terrains):
            for i in range(num_levels):
                ...
                discrete_obstacles_height = 0.025 + difficulty * 0.10
                stepping_stones_size = 2 - 1.8 * difficulty
                rough = 0.1 * difficulty + 0.02
                if choice < self.proportions[0]:
                    ...
                    if choice < 0.15:
                        slope *= -1
                    pyramid_sloped_terrain(terrain, slope=slope, platform_size=3.)
                    random_uniform_terrain(terrain, min_height=-rough, max_height=rough, step=0.025, downsampled_scale=0.2)
